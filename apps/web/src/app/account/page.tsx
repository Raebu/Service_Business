...
export default async function AccountPage(){
  ...
  const [{data:profile},{data:memberships},{data:learner}]=await Promise.all([
    ...
    supabase!.from('learners').select('id,display_name,current_stage,status').eq('user_id',user.id).maybeSingle()
  ]);
  ...
  return <section className='page'><div className='account-header'><div><span className='eyebrow'>Your account</span><h1>Welcome{profile?.display_name?`, ${profile.display_name}`:''}.</h1><p className='lede'>Choose the workspace you need. You only see organisations and records linked to this signed-in account.</p></div><SignOutButton/></div><div className='cards three workspace-cards'><article><span>Personal</span><h2>Customer workspace</h2><p>Your electrical jobs, properties and service records.</p><Link className='button primary' href='/portal/customer'>Open customer workspace</Link></article>{learner&&learner.status!=='withdrawn'&&<article><span>Academy learner</span><h2>{learner.display_name}</h2><p>Review consented employer matches, record your interest and follow supervised placements.</p><Link className='button primary' href='/portal/academy/learner'>Open my Academy</Link></article>}{providerOrgs.map(m=><article key={m.organisations!.id}><span>Electrical business</span><h2>{m.organisations!.name}</h2><p>Work offers, compliance, coverage, verification and performance.</p><Link className='button primary' href={`/portal/provider?organisation=${m.organisations!.id}`}>Open contractor portal</Link></article>)}{businessOrgs.map(m=><article key={m.organisations!.id}><span>Business client</span><h2>{m.organisations!.name}</h2><p>Portfolio jobs, properties, compliance and reporting.</p><Link className='button primary' href={`/portal/business?organisation=${m.organisations!.id}`}>Open business portal</Link></article>)}{educationOrgs.map(m=><article key={m.organisations!.id}><span>Education partner</span><h2>{m.organisations!.name}</h2><p>Employer engagement, placements and Academy activity.</p><Link className='button primary' href={`/portal/academy?organisation=${m.organisations!.id}`}>Open Academy workspace</Link></article>)}{profile?.default_role==='admin'&&<article><span>Internal</span><h2>Operations control plane</h2><p>Verification, coverage, live jobs, service recovery and corporate onboarding.</p><Link className='button primary' href='/ops'>Open operations</Link></article>}</div>{typedMemberships.length===0&&!learner&&profile?.default_role!=='admin'&&<p className='note'>No organisation workspace is linked yet. Your customer workspace is still available; verified provider and business memberships appear here when onboarding is completed.</p>}</section>;
}
