...
export async function POST(request:Request,{params}:{params:Promise<{jobId:string}>}){
  ...
  try{
    ...
    const {data:job,error}=await supabase.from('jobs').select('id,status,service_key,provider_price_pence,platform_fee_pence,customer_total_pence,currency,payment_status,stripe_checkout_session_id,email,customer_name,matched_provider_id').eq('id',jobId).maybeSingle();
    ...
    const session=await stripe.checkout.sessions.create({
      ...
      payment_intent_data:{transfer_group:transferGroup,metadata:{job_id:jobId,provider_id:job.matched_provider_id}},
      metadata:{job_id:jobId,provider_id:job.matched_provider_id,provider_price_pence:String(job.provider_price_pence),platform_fee_pence:String(job.platform_fee_pence)},
    ...
    await supabase.from('audit_events').insert({event_type:'payment.checkout_created',entity_type:'job',entity_id:jobId,metadata:{checkoutSessionId:session.id,customerTotalPence:job.customer_total_pence,providerPricePence:job.provider_price_pence,platformFeePence:job.platform_fee_pence}});
