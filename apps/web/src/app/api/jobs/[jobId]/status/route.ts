...
export async function GET(request:Request,{params}:{params:Promise<{jobId:string}>}){
  try{
    ...
    const [{data:cases},{data:scope},{data:followOnRows}]=await Promise.all([
      ...
      supabase.from('follow_on_quotes').select('id,description,provider_price_pence,platform_fee_pence,customer_total_pence,status,expires_at,approved_at,payment_status,settlement_status,paid_at,created_at').eq('job_id',jobId).order('created_at',{ascending:false}).limit(5)
    ]);
    ...
    const followOn=(followOnRows||[]).map(q=>({id:q.id,description:q.description,providerPricePence:Number(q.provider_price_pence),platformFeePence:Number(q.platform_fee_pence),customerTotalPence:Number(q.customer_total_pence),status:q.status,expiresAt:q.expires_at,approvedAt:q.approved_at,paymentStatus:q.payment_status,settlementStatus:q.settlement_status,paidAt:q.paid_at}));
