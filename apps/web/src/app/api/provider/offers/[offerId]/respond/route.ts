...
export async function POST(request:Request,{params}:{params:Promise<{offerId:string}>}){
  ...
  const {data,error}=await supabase.rpc('respond_to_job_offer',{p_offer_id:offerId,p_provider_id:provider.id,p_action:parsed.data.action});
  if(error){
    const conflict=['offer_not_available','offer_expired','job_not_available','job_not_owned_by_provider','job_not_completable'].some(code=>error.message.includes(code));
