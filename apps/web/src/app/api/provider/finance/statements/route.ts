...
export async function GET(request:Request){
 ...
 const [{data:jobs},{data:adjustments}]=await Promise.all([
  admin.from('jobs').select('id,created_at,postcode,status,provider_price_pence,platform_fee_pence,customer_total_pence,payment_status,settlement_status,stripe_processing_fee_pence,net_platform_margin_pence,stripe_transfer_id').eq('matched_provider_id',provider.id).order('created_at',{ascending:true}),
  admin.from('payment_adjustments').select('job_id,adjustment_type,amount_pence,reason,status,created_at').order('created_at',{ascending:true})
 ]);
 const rows=['record_type,date,job_id,postcode,job_status,provider_entitlement_pence,platform_fee_pence,customer_total_pence,payment_status,settlement_status,stripe_fee_pence,platform_margin_pence,adjustment_type,adjustment_pence,adjustment_reason'];
 for(const j of jobs||[]){rows.push(['job',j.created_at,j.id,j.postcode,j.status,j.provider_price_pence,j.platform_fee_pence,j.customer_total_pence,j.payment_status,j.settlement_status,j.stripe_processing_fee_pence,j.net_platform_margin_pence,'','',''].map(esc).join(','));for(const a of (adjustments||[]).filter(x=>x.job_id===j.id))rows.push(['adjustment',a.created_at,j.id,j.postcode,j.status,'','','',j.payment_status,j.settlement_status,'','',a.adjustment_type,a.amount_pence,a.reason].map(esc).join(','))}
