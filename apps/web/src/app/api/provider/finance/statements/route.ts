...
export async function GET(request:Request){
 const url=new URL(request.url);const organisation=url.searchParams.get('organisation');const from=url.searchParams.get('from');const to=url.searchParams.get('to');if(!organisation)return NextResponse.json({error:'organisation is required'},{status:400});
 ...
 let jobsQuery=admin.from('jobs').select('id,created_at,postcode,status,provider_price_pence,platform_fee_pence,customer_total_pence,payment_status,settlement_status,stripe_processing_fee_pence,net_platform_margin_pence,stripe_transfer_id').eq('matched_provider_id',provider.id).order('created_at',{ascending:true});
 if(from)jobsQuery=jobsQuery.gte('created_at',`${from}T00:00:00.000Z`);if(to)jobsQuery=jobsQuery.lte('created_at',`${to}T23:59:59.999Z`);const {data:jobs}=await jobsQuery;
 const jobIds=(jobs||[]).map(j=>j.id);const [{data:adjustments},{data:reserves}]=await Promise.all([
  ...
  jobIds.length?admin.from('provider_reserve_holds').select('job_id,amount_pence,status,reason,release_at,released_at,created_at').eq('provider_id',provider.id).in('job_id',jobIds).order('created_at',{ascending:true}):Promise.resolve({data:[]})
 ]);
 ...
 const rangeSuffix=from||to?`-${from||'start'}-to-${to||'present'}`:'';const body=rows.join('\n');return new NextResponse(body,{headers:{'content-type':'text/csv; charset=utf-8','content-disposition':`attachment; filename="provider-finance-${organisation}${rangeSuffix}.csv"`,'cache-control':'no-store'}})
