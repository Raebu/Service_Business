...
export async function POST(request:Request){
  ...
  try{
    ...
    const {data:profiles,error}=await supabase.from('corporate_billing_profiles').select('organisation_id,billing_cycle,payment_terms_days,purchase_order_required,consolidated_billing,billing_email').eq('consolidated_billing',true);
    if(error)return NextResponse.json({error:'Unable to load billing profiles.'},{status:500});
    const results:Array<Record<string,unknown>>=[];
    for(const profile of profiles||[]){
      const {data:jobs}=await supabase.from('jobs').select('id,customer_total_pence,platform_fee_pence,corporate_sla_id,po_reference,cost_centre,site_reference,status,completed_at,created_at').eq('business_organisation_id',profile.organisation_id).gte('created_at',`${start}T00:00:00Z`).lte('created_at',`${end}T23:59:59Z`).in('status',['completed','closed']);
      const rows=jobs||[];
      ...
      const slaIds=[...new Set(rows.map(j=>j.corporate_sla_id).filter(Boolean))];
