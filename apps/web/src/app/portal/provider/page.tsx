...
export default async function ProviderPortalPage({searchParams}:{searchParams:Promise<{organisation?:string}>}){
  ...
  const {data:jobs}=jobIds.length?await supabase.from('jobs').select('id,status,postcode,address,description,urgency,preferred_window').in('id',jobIds):{data:[] as Array<{id:string;status:string;postcode:string;address:string;description:string;urgency:string;preferred_window:string|null}>};
  const jobMap=new Map((jobs||[]).map(j=>[j.id,j]));
  return <section className='page'><div className='portal-heading'><div><span className='eyebrow'>Contractor portal</span><h1>{org.name}</h1><p className='lede'>Work, compliance, coverage and your network trust status in one place.</p></div><div className='actions'><Link className='button' href='/account'>Back to account</Link><Link className='button primary' href={`/verify/${provider.public_slug}`}>View public verification</Link></div></div><div className='metrics'><div><strong>{provider.verification_state}</strong><span>verification</span></div><div><strong>{Number(provider.quality_score).toFixed(0)}</strong><span>quality score</span></div><div><strong>{Math.round(Number(provider.acceptance_rate)*100)}%</strong><span>acceptance</span></div><div><strong>{Math.round(Number(provider.completion_rate)*100)}%</strong><span>completion</span></div></div><div className='portal-grid'><section className='portal-card'><h2>Work offers</h2>{!offers?.length?<p>No work offers yet.</p>:offers.map(offer=>{const job=jobMap.get(offer.job_id);return <article className='offer-record' key={offer.id}><div className='record-row'><div><strong>{job?.description||'Electrical work'}</strong><span>{job?`${job.postcode} · ${job.urgency}${job.preferred_window?` · ${job.preferred_window}`:''}`:'Job details unavailable'}</span></div><span className='status-pill'>{offer.status}</span></div><ProviderOfferActions offerId={offer.id} status={offer.status}/></article>})}</section><section className='portal-card'><h2>Compliance evidence</h2>{!evidence?.length?<p>No evidence records yet.</p>:evidence.map(item=><article className='record-row' key={item.id}><div><strong>{item.label}</strong><span>{item.expires_at?`Expires ${new Date(item.expires_at).toLocaleDateString('en-GB')}`:item.kind}</span></div><span className='status-pill'>{item.status}</span></article>)}</section><section className='portal-card'><h2>Coverage</h2>{!coverage?.length?<p>No coverage areas configured yet.</p>:<div className='chips'>{coverage.filter(x=>x.active).map(x=><span key={x.area}>{x.area}</span>)}</div>}</section><section className='portal-card'><h2>Services</h2>{!services?.length?<p>No services configured yet.</p>:<div className='chips'>{services.filter(x=>x.active).map(x=><span key={x.service_key}>{x.service_key}</span>)}</div>}</section></div></section>;
}
