...
export default async function ProviderTeamPage({searchParams}:{searchParams:Promise<{organisation?:string}>}){
  ...
  return <section className='page'><div className='portal-heading'><div><span className='eyebrow'>Electrical business team</span><h1>{org.name}</h1><p className='lede'>Individual engineer identities, skills, working hours, time off, calendar export/import, location and auto-accept rules. Dispatch can only treat someone as independently eligible after verified competency allows unsupervised work.</p></div><div className='actions'><Link className='button' href={`/portal/provider?organisation=${organisation}`}>Back to contractor portal</Link><Link className='button' href={`/portal/provider/pricing?organisation=${organisation}`}>Pricing</Link></div></div><div className='portal-grid'><section className='portal-card'><h2>People</h2>{engineers.length===0?<p>No engineers have been added yet.</p>:engineers.map(engineer=>{const engineerRules=availability.filter(a=>a.engineer_id===engineer.id);const engineerTimeOff=timeOff.filter(t=>t.engineer_id===engineer.id);const canEditSchedule=canSchedule||engineer.user_id===user.id;return <article className='ops-record' key={engineer.id}><div className='record-row'><div><strong>{engineer.display_name}</strong><span>{engineer.employment_role} · {engineer.email||'No email'} · {engineer.user_id?'login linked':'login pending'}</span></div><span className='status-pill'>{engineer.can_work_unsupervised?'independent':'supervised only'}</span></div><div className='actions'><a className='button' href={`/api/provider/engineers/${engineer.id}/calendar.ics`}>Export calendar (.ics)</a></div>{canManage&&!engineer.user_id&&engineer.status==='invited'&&<InvitationControls engineerId={engineer.id}/>}<div className='chips'>{competencies.filter(c=>c.engineer_id===engineer.id).map(c=><span key={c.id}>{c.service_key}: {c.competency_level} · {c.verified?'verified':'pending'}</span>)}</div>{engineerRules.length>0&&<div className='schedule-summary'>{engineerRules.map(rule=><span key={rule.id}><strong>{dayNames[rule.day_of_week]} {rule.start_time.slice(0,5)}–{rule.end_time.slice(0,5)}</strong>{rule.auto_accept?' · auto-accept':''}{rule.maximum_travel_minutes?` · ≤${rule.maximum_travel_minutes} min travel`:''}{` · ${rule.buffer_before_minutes}/${rule.buffer_after_minutes} min buffers`}{rule.maximum_jobs_per_day?` · ≤${rule.maximum_jobs_per_day} jobs`:''}{rule.allowed_service_keys?.length?` · ${rule.allowed_service_keys.join(', ')}`:''}</span>)}</div>}{canEditSchedule&&<TimeOffManager engineerId={engineer.id} initial={engineerTimeOff}/>} {canEditSchedule&&<CalendarConnectionManager engineerId={engineer.id}/>} {engineer.user_id===user.id&&<LiveLocationControl engineerId={engineer.id}/>} {(canManage||engineer.user_id===user.id)&&<CompetencyForm engineerId={engineer.id}/>} {canEditSchedule&&<AvailabilityForm engineerId={engineer.id}/>}</article>})}</section>{canManage&&<TeamManager organisationId={organisation}/>}</div><p className='note'>Apprentice and trainee roles are always prevented at database level from being marked as unsupervised. Other engineers only become independently dispatchable once a competent/advanced skill has been verified and is still in date. Time off and imported external busy blocks are included in availability checks. Calendar interoperability is optional; iCalendar feeds are read-only busy-time inputs and do not make the platform depend on an external calendar. Precise live location is opt-in and short-lived.</p></section>;
}
