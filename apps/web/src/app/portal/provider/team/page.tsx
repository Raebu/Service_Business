...
import { TeamManager,CompetencyForm,AvailabilityForm } from './TeamManager';
...
type Availability={id:string;engineer_id:string;day_of_week:number;start_time:string;end_time:string;auto_accept:boolean;minimum_job_pence:number;maximum_duration_minutes:number|null;maximum_travel_minutes:number|null};
...
export default async function ProviderTeamPage({searchParams}:{searchParams:Promise<{organisation?:string}>}){
  ...
  const [{data:competencyData},{data:availabilityData}]=ids.length?await Promise.all([
    ...
    supabase.from('engineer_availability_rules').select('id,engineer_id,day_of_week,start_time,end_time,auto_accept,minimum_job_pence,maximum_duration_minutes,maximum_travel_minutes').in('engineer_id',ids).eq('active',true).order('day_of_week')
  ...
  return <section className='page'><div className='portal-heading'><div><span className='eyebrow'>Electrical business team</span><h1>{org.name}</h1><p className='lede'>Individual engineer identities, skills, calendars and auto-accept rules. Dispatch can only treat someone as independently eligible after verified competency allows unsupervised work.</p></div><div className='actions'><Link className='button' href={`/portal/provider?organisation=${organisation}`}>Back to contractor portal</Link><Link className='button' href={`/portal/provider/pricing?organisation=${organisation}`}>Pricing</Link></div></div><div className='portal-grid'><section className='portal-card'><h2>People</h2>{engineers.length===0?<p>No engineers have been added yet.</p>:engineers.map(engineer=>{const engineerRules=availability.filter(a=>a.engineer_id===engineer.id);return <article className='ops-record' key={engineer.id}><div className='record-row'><div><strong>{engineer.display_name}</strong><span>{engineer.employment_role} · {engineer.email||'No email'} · {engineer.user_id?'login linked':'login pending'}</span></div><span className='status-pill'>{engineer.can_work_unsupervised?'independent':'supervised only'}</span></div><div className='chips'>{competencies.filter(c=>c.engineer_id===engineer.id).map(c=><span key={c.id}>{c.service_key}: {c.competency_level} · {c.verified?'verified':'pending'}</span>)}</div>{engineerRules.length>0&&<div className='schedule-summary'>{engineerRules.map(rule=><span key={rule.id}><strong>{dayNames[rule.day_of_week]} {rule.start_time.slice(0,5)}–{rule.end_time.slice(0,5)}</strong>{rule.auto_accept?' · auto-accept':''}{rule.maximum_travel_minutes?` · ≤${rule.maximum_travel_minutes} min travel`:''}</span>)}</div>}{(canManage||engineer.user_id===user.id)&&<CompetencyForm engineerId={engineer.id}/>} {(canSchedule||engineer.user_id===user.id)&&<AvailabilityForm engineerId={engineer.id}/>}</article>})}</section>{canManage&&<TeamManager organisationId={organisation}/>}</div><p className='note'>Apprentice and trainee roles are always prevented at database level from being marked as unsupervised. Other engineers only become independently dispatchable once a competent/advanced skill has been verified and is still in date. Auto-accept is a scheduling preference, not a safety bypass.</p></section>;
}
