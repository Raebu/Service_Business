...
import { TravelBandEditor } from './TravelBandEditor';

export const dynamic='force-dynamic';

type TravelBand={id:string;service_key:string|null;minimum_distance_meters:number;maximum_distance_meters:number|null;charge_pence:number;reject_beyond_band:boolean};
...
export default async function ProviderPricingPage({searchParams}:{searchParams:Promise<{organisation?:string}>}){
  ...
  const [{data:cards},{data:travelBands}]=await Promise.all([
    supabase.from('provider_rate_cards').select('id,name,version,currency,effective_from').eq('organisation_id',organisation).eq('active',true).order('version',{ascending:false}),
    supabase.from('provider_travel_bands').select('id,service_key,minimum_distance_meters,maximum_distance_meters,charge_pence,reject_beyond_band').eq('organisation_id',organisation).eq('active',true).order('minimum_distance_meters')
  ]);
